...
public class ModernJavaApp {
    ...
    public static void main(String[] args) {
        ...
        JavalinValidation
            .register(URI.class, s -> HttpRequest.newBuilder()
                .uri(URI.create(s))
                .build()
                .uri()
            );
        ...
        final var app = Javalin
            .create(javalinConfig -> javalinConfig.http.asyncTimeout = 10_000)
            .start(7070);

        app.get("/async-proxy/<url>", ctx -> {
            final var validUri = ctx.pathParamAsClass("url", URI.class).get();
            final var request = HttpRequest.newBuilder().GET().uri(validUri).build();
